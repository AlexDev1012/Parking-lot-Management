...
const GrabberItem = ({ side }: { side: "left" | "right" }) => {
  return (
    <div
      className="absolute w-[9px] h-2/5 px-[3px] py-1 bg-white rounded-full shadow-[0_0_2px_2px_#8888] top-[50%]"
      style={{
        [side]: 0,
...
const RangeItem = ({
  ...
  return (
    <div
      className={`h-full border-x pointer-events-none select-none ${
        editing
          ? "border-[#cf6262] bg-[#aa505080]"
          : "border-[#62cb66] bg-[#62cb6680]"
      }  absolute group`}
      style={{
        left: `${range.start}px`,
...
const RangeSelector = ({
  ...
  const [scaledRanges, setScaledRanges] = useState<
  ...
  const convert2NumberRange = (v: { start: Date; end: Date }) => ({
    start: zoomedScale.current(v.start),
    end: zoomedScale.current(v.end),
  });
  ...
  const zoomHandler = (event: any) => {
    ...
    const time_scale = d3
      .scaleTime()
      .domain([
        new Date(zoomedScale.current.invert(0)),
        new Date(zoomedScale.current.invert(w_wrapper.current)),
      ]) // This would be your start and end dates
      .rangeRound([0, w_wrapper.current]);
    ref_gX.current?.call(d3.axisBottom(time_scale));
  };
  const zoom = d3.zoom().scaleExtent([0.5, 3000]).on("zoom", zoomHandler);
  const enableZoom = () => {
  ...
  const disableZoom = () => {
    if (!zoomEnabled) return;
    if (user?.customClaims.level !== 1) return;
  ...
  const zoomTo = (date: Date) => {
    ...
    d3.select(ref_wrapper.current as Element)
      ...
      .call(
        zoom.transform,
        d3.zoomIdentity
          .translate(-w_wrapper.current * elapsedDays, 0)
          .scale(isLeapYear ? 366 : 365)
      );
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (ref_wrapper.current && ref_axis_wrapper.current) {
      ...
      setScaledRanges(ranges.map((v) => convert2NumberRange(v)));
      const tick_svg = d3
        .select(ref_axis_wrapper.current)
        .append("svg")
      ...
      const time_scale = d3
        ...
        .rangeRound([0, w_wrapper.current]);
      ref_gX.current = tick_svg
        .append("g")
        .attr(
          "transform",
          "translate(0," + (ref_axis_wrapper.current.clientHeight / 2) * 0 + ")"
        );
      ref_gX.current.call(d3.axisBottom(time_scale));
      enableZoom();
      ...
      d3.select(ref_wrapper.current as Element)
        .on("dblclick", function (event) {
          ...
          const hoverRange = ref_ranges.current.find(
            (r) => r.start < curTime && curTime < r.end
          );
          if (hoverRange) {
        ...
        .on("mousemove", function (event) {
          ...
          if (!ref_zoomEnabled.current && ref_mouseDown.current) {
            const indicating_val = new Date(
              zoomedScale.current.invert(coordinates[0])
            );
            const hoverRange = ref_ranges.current.find(
              (r) => r.start < indicating_val && indicating_val < r.end
            );
            ...
            setEditingRange((v) => {
              ...
              if (ref_grab.current === "LEFT") {
                if (
                  ref_ranges.current.find(
                    (r) => indicating_val < r.start && r.end < v.end
                  )
                )
                  return v;
                return {
                  start: indicating_val,
                    ...
                    v.end.getTime() - indicating_val.getTime() < 30 * 60_000
                      ? new Date(indicating_val.getTime() + 30 * 60_000)
                      : v.end,
                };
              } else {
                if (
                  ref_ranges.current.find(
                    (r) => v.start < r.start && r.end < indicating_val
                  )
                )
                  return v;
                return {
                  start:
                    v.end.getTime() > indicating_val.getTime()
                      ? indicating_val.getTime() - v.start.getTime() <
                        30 * 60_000
                        ? new Date(indicating_val.getTime() - 30 * 60_000)
                        : v.start
                      : v.start,
        ...
        .on("mouseout", function (event) {
          ...
          if (ref_mouseDown.current && !ref_zoomEnabled.current)
            ref_wrapper.current?.dispatchEvent(
        ...
        .on("mousedown", function (event) {
          ...
          if (hoverRange) {
            ...
            setGrab(
              curTime.getTime() - hoverRange.start.getTime() <
                hoverRange.end.getTime() - curTime.getTime()
                ? "LEFT"
                : "RIGHT"
            );
  ...
  return (
    <>
      <h1 className="w-full text-2xl font-bold text-center p-4">
        <span>
          {d3.timeFormat("%Y-%m-%d")(new Date(zoomedScale.current.invert(0)))}
        </span>
        <small> To </small>
        <span>
          {d3.timeFormat("%Y-%m-%d")(
            new Date(
              zoomedScale.current.invert(ref_wrapper.current?.clientWidth || 0)
            )
          )}
        </span>
      </h1>
      <div className="w-full">
        <div className="w-fit flex items-center border border-black mb-3">
          <button
            className={`p-2 ${
              zoomEnabled ? "bg-[#3b2dff] text-white" : ""
            } hover:scale-[1.2]`}
            onClick={enableZoom}
          >
          ...
          <button
            disabled={user?.customClaims.level !== 1}
            className={`p-2 ${
              zoomEnabled ? "" : "bg-[#3b2dff] text-white"
            } hover:scale-[1.2] disabled:cursor-not-allowed disabled:opacity-30`}
            onClick={disableZoom}
          >
            <svg className="w-4 h-4 fill-current">
              <use href="#svg-edit" />
        ...
        <div className="relative">
          <div
            ref={ref_wrapper}
            className={`w-full relative h-[50px] border border-black/10 ${
              zoomEnabled ? "cursor-move" : "cursor-pointer"
            } overflow-hidden`}
          >
            ...
            {editingRange && (
              <RangeItem
                range={convert2NumberRange(editingRange)}
                zoomEnabled={false}
                editing
              />
            )}
          ...
          <div
            className={`w-[1px] h-full bg-black absolute top-0 select-none pointer-events-none ${
              indicatorX < 0 ? "hidden" : ""
            }`}
            style={{ left: indicatorX }}
          />
          <div
            className={`w-fit whitespace-nowrap select-none pointer-events-none absolute top-0 -translate-x-[50%] -translate-y-[100%] text-[10px] ${
              indicatorX < 0 ? "hidden" : ""
            }`}
            style={{ left: indicatorX }}
          >
            {d3.timeFormat("%Y-%m-%d %H:%M")(
              new Date(zoomedScale.current.invert(indicatorX))
            )}
          </div>
        </div>
        <div
          ref={ref_axis_wrapper}
          className="w-full h-[50px] relative pointer-events-none-all"
        />
      </div>
      <Toast ref={toast} />
    </>
